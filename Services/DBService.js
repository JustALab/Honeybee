...
export const DBService = {
    ...
    initDB: () => {
        ...
        db.transaction((tx) => {
            ...
            tx.executeSql('CREATE TABLE IF NOT EXISTS user_data (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, first_name VARCHAR(50), last_name VARCHAR(50), email VARCHAR(200) NOT NULL, mobile VARCHAR(10), is_logged_in INTEGER DEFAULT 0, token TEXT)', [], (tx, res) => {
    ...
    insertIntoUserData: (email, token) => {
        db.transaction((tx) => {
            tx.executeSql('SELECT * FROM user_data', 
            [], 
            (tx, res) => {
                if(res.rows.length === 0) {
                    let isLoggedIn = 1;
                    tx.executeSql('INSERT INTO user_data (email, is_logged_in, token) VALUES (:email, :isLoggedIn, :token)', 
                    [email, isLoggedIn, token], 
                    (tx) => {
                        console.log('user data inserted successfully.');
                    }, err => {
                        console.log('user data insertion not successfull: ' + err.message);
                    });
                } else {
    ...
    updateUserData: (email, token) => {
        db.transaction((tx) => {
            tx.executeSql('UPDATE user_data SET email=(:email), token=(:token) WHERE id=(:id)',
                [email, token, 1],
                (tx) => {
